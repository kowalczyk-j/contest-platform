...
const GradeEntry = () => {
  ...
  const handleGradeFetch = async (event) => {
    ...
    try {
      ...
      const entryResponse = await axios.get(`${import.meta.env.VITE_API_URL}api/entries/${entryId}`, headers);
      const entry = entryResponse.data;
      setEntry(entry);

      const gradeResponse = await axios.get(`${import.meta.env.VITE_API_URL}api/grades/?entry_id=${entryId}`, headers);
      const grades = gradeResponse.data;

      const gradeCriterionsPromises = grades.map(async (grade) => {
        const criterionResponse = await axios.get(`${import.meta.env.VITE_API_URL}api/criterions/${grade.criterion}`, headers);
  ...
  const handleGradeUpload = async (updatedGradesAndCriterions) => {
    ...
    try {
      ...
      for (const pair of updatedGradesAndCriterions) {
        await axios.patch(`${import.meta.env.VITE_API_URL}api/grades/${pair.grade.id}/`, pair.grade, headers);
      }
    ...
    <ThemeProvider theme={montserrat}>
      <div>
        ...
        <Grid container justifyContent="center" alignItems="center">
          <GradeEntryForm entryName={entry.entry_title} authorName={entry.user} age={entry.user} applicant={entry.user} gradesAndCriterions={gradesAndCriterions} handleGradeUpload={handleGradeUpload} />
        </Grid>
        <ConfirmationWindow
          open={openPopup}
          setOpen={setOpenPopup}
          title={fetchError ? "Nie udało się pobrać zgłoszenia" : (uploadError ? "Nie udało się zapisać ocen" : "Pomyślnie zapisano oceny")}
          message={fetchError ? fetchErrorMessage : (uploadError ? uploadErrorMessage : null)}
