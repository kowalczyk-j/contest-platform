  ...
  useEffect(() => {
    ...
    axios
      .get(
        ...
        }api/contests/${contestId}/group_individual_comp/`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Token " + sessionStorage.getItem("accessToken"),
    ...
    axios
      .get(
        `${
          import.meta.env.VITE_API_URL
        }api/contests/${contestId}/get_contestants_amount/`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Token " + sessionStorage.getItem("accessToken"),
      ...
      .catch((error) =>
        console.error("Error fetching contestant amount: ", error)
      );
    ...
    axios
      .get(
        `${
          import.meta.env.VITE_API_URL
        }api/contests/${contestId}/get_submissions_by_day/`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Token " + sessionStorage.getItem("accessToken"),
    ...
    axios
      .get(
        `${
          import.meta.env.VITE_API_URL
        }api/contests/${contestId}/get_entry_amount/`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Token " + sessionStorage.getItem("accessToken"),
      ...
      <ThemeProvider theme={montserrat}>
        ...
        >
          ...
          >
            ...
            <Typography
              variant="subtitle1"
              sx={{ mt: 1, mx: 5, alignItems: "center" }}
            ...
            >
                ...
                <Doughnut
                  data={{
                    ...
                    datasets: [
                      {
                        data: [
                          submissionTypeCount.solo_entries,
                          submissionTypeCount.group_entries,
                        ],
                        backgroundColor: [
                          "rgba(43, 63, 229, 0.8)",
                          "rgba(250, 192, 19, 0.8)",
              ...
              <Box sx={{ flex: 2, paddingLeft: 2 }}>
                {dailySubmissions.daily_entries.length > 0 && (
                  <Line
                    data={{
                      labels: dailySubmissions.daily_entries.map(
                        (entry) => entry.date_submitted
                      ),
                      datasets: [
                        {
                          label: "Ilość prac",
                          data: dailySubmissions.daily_entries.map(
