...
function CreateCriterion({ index, onCriterionChange, onCriterionRemove }) {
    ...
    useEffect(() => {
        onCriterionChange(index, { description, max_rating: maxRating, user: selectedOption });
        ...
        axios
            ...
            .then(response => {
                ...
                const options = users.map(user => ({
                    value: user.id, // Ustawiamy id jako value
                    label: user.username // Ustawiamy username jako label
                }));
